...
public class StoryJobScheduler {
    ...
    public void syncAndResetStoryViews() {
        log.info(
                "time={} - service=story-job-scheduler - info=job - message=Syncing and resetting story views...",
    ...
    public void updateHotStory() {
        log.info(
                "time={} - service=story-job-scheduler - info=job - message=Updating hot stories...",
                java.time.LocalDateTime.now());
        ...
        if (!stories.isEmpty()) {
            ...
            log.info(
                    "time={} - service=story-job-scheduler - info=job - message=Updated {} stories to hot status",
                    java.time.LocalDateTime.now(),
                    stories.size());
        } else {
            log.debug(
                    "time={} - service=story-job-scheduler - info=job - message=No stories to update to hot status",
                    java.time.LocalDateTime.now());
        }
    }
    ...
    private void updateStoryViewCountInMySQL(Integer storyId, String viewCountStr) {
        ...
        if (story != null) {
            ...
            log.debug(
                    "time={} - service=story-job-scheduler - info=job - message=Updated story {} views count by {}",
                    java.time.LocalDateTime.now(),
                    storyId,
                    viewCount);
        }
