...
public class AuthenticationService {
    ...
    public String generateToken(User user, boolean isRefresh) {
        ...
        try {
            ...
            log.error(
                    "time={} - service=authentication-service - info=token - message=Cannot create token",
    ...
    private SignedJWT verifyToken(String token, boolean isRefresh) {
        try {
            ...
            log.error(
                    "time={} - service=authentication-service - info=token - message=Token verification failed",
                    java.time.LocalDateTime.now(),
                    e);
            throw new AppException(ErrorCode.INVALID_TOKEN);
        } catch (Exception e) {
            log.error(
                    "time={} - service=authentication-service - info=token - message=Unexpected error during token verification",
                    java.time.LocalDateTime.now(),
                    e);
            throw new AppException(ErrorCode.UNAUTHENTICATED);
        }
