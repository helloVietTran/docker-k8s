...
/**
 * quản lý Distributed Lock trên Redis
...
public class RedisLockService {
    ...
    public String tryAcquireLock(String lockKey, long timeoutSeconds) {
        ...
        Boolean acquired =
                stringRedisTemplate.opsForValue().setIfAbsent(lockKey, lockValue, Duration.ofSeconds(timeoutSeconds));

        if (acquired != null && acquired) {
            log.info(
                    "time={} - service=redis-lock - info=lock - message=Lock acquired: {} (timeout: {}s)",
        ...
        log.warn(
                "time={} - service=redis-lock - info=lock - message=Lock not acquired (already locked): {}",
                java.time.LocalDateTime.now(),
                lockKey);
        return null;
    }
    ...
    public boolean releaseLock(String lockKey, String lockValue) {
        ...
        if (lockValue.equals(currentValue)) {
            ...
            log.info(
                    "time={} - service=redis-lock - info=lock - message=Lock released: {}",
                    java.time.LocalDateTime.now(),
                    lockKey);
            return true;
        }
        ...
        log.warn(
                "time={} - service=redis-lock - info=lock - message=Lock value mismatch, cannot release: {}",
                java.time.LocalDateTime.now(),
                lockKey);
        return false;
    }
    ...
    public boolean executeWithLock(String lockKey, long timeoutSeconds, Runnable task) {
        ...
        try {
            ...
            log.error(
                    "time={} - service=redis-lock - info=lock - message=Error while executing task with lock: {}",
                    java.time.LocalDateTime.now(),
                    lockKey,
                    e);
            return false;
